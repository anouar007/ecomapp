            ...
            <?php if($product->track_inventory): ?>
                            ...
                            <div class="variant-card mb-2 d-flex align-items-center justify-content-between variant-container" 
                                ...
                                <div class="d-flex align-items-center gap-2 variant-info-wrapper">
                                    <div class="d-flex align-items-center gap-2">
                                        <?php if($variant->color_image): ?>
                                            <img src="<?php echo e($variant->color_image_url); ?>" class="variant-thumb" alt="">
                                        <?php elseif($variant->color): ?>
                                            <span class="color-dot shadow-inner" style="width: 20px; height: 20px; border-radius: 50%; background: <?php echo e($variant->color); ?>"></span>
                                        <?php endif; ?>
                                        
                                        <?php if($variant->size): ?>
                                            <span class="text-dark fw-bold font-inter ms-1" style="font-size: 0.85rem;"><?php echo e($variant->size); ?></span>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <div class="variant-actions-wrapper">
                                    <div class="stock-stepper">
                                        <button type="button" class="quick-adj-btn" onclick="quickUpdate('<?php echo e($product->id); ?>', '<?php echo e($variant->id); ?>', 1)">
                                            <i class="fas fa-plus"></i>
                                        </button>
                                        <input type="number" 
                                               class="stock-input variant-stock-display <?php echo e(($variant->stock ?? 0) <= 0 ? 'out' : (($variant->stock ?? 0) <= 5 ? 'low' : 'in')); ?>" 
                                               value="<?php echo e($variant->stock ?? 0); ?>"
                                               onchange="saveQuantity('<?php echo e($product->id); ?>', '<?php echo e($variant->id); ?>', this)">
                                        <button type="button" class="quick-adj-btn" onclick="quickUpdate('<?php echo e($product->id); ?>', '<?php echo e($variant->id); ?>', -1)">
                ...
                <?php else: ?>
                    <div class="d-flex align-items-center justify-content-between p-2 rounded-3" style="background: #f8fafc;">
                        ...
                        <div class="stock-stepper">
                            <button type="button" class="quick-adj-btn" onclick="quickUpdate('<?php echo e($product->id); ?>', null, 1)">
                                <i class="fas fa-plus"></i>
                            </button>
                            <input type="number" 
                                   class="stock-input product-stock-display <?php echo e(($product->stock ?? 0) <= 0 ? 'out' : (($product->stock ?? 0) <= 5 ? 'low' : 'in')); ?>" 
                                   value="<?php echo e($product->stock ?? 0); ?>"
                                   onchange="saveQuantity('<?php echo e($product->id); ?>', null, this)">
                            <button type="button" class="quick-adj-btn" onclick="quickUpdate('<?php echo e($product->id); ?>', null, -1)">
                                <i class="fas fa-minus"></i>
                            </button>
                        </div>
                                ...
                                <?php if($product->track_inventory): ?>
                                        <?php if($product->variants->count() == 0): ?>
                                            <div class="stock-stepper">
                                                ...
                                                <input type="number" 
                                                       class="stock-input product-total-stock-display <?php echo e(($product->total_stock <= ($product->low_stock_threshold ?? 10)) ? 'out' : 'in'); ?>" 
                                                       value="<?php echo e($product->total_stock); ?>"
                                                       onchange="saveQuantity('<?php echo e($product->id); ?>', null, this)">
                                                <button type="button" class="quick-adj-btn" onclick="quickUpdate('<?php echo e($product->id); ?>', null, -1)">
                                                    <i class="fas fa-minus"></i>
                                                </button>
...
<script>
    ...
    async function quickUpdate(productId, variantId, change) {
        ...
        try {
            ...
            if (data.success) {
                ...
                showError("<?php echo e(__('Update Failed')); ?>", data.message || "<?php echo e(__('Update failed')); ?>");
            }
        } catch (error) {
            input.value = originalVal; // Revert
            console.error(error);
            showError("<?php echo e(__('Error')); ?>", "<?php echo e(__('Something went wrong')); ?>");
        } finally {
            btn.disabled = false;
        }
    }
    ...
    async function saveQuantity(productId, variantId, input) {
        ...
        if (isNaN(newVal) || newVal < 0) {
            showError("<?php echo e(__('Invalid Quantity')); ?>", "<?php echo e(__('Please enter a valid positive number')); ?>");
            return;
        }
