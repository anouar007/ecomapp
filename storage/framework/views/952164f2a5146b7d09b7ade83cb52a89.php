...
<div style="padding: 24px; max-width: 800px; margin: 0 auto;">
    <div style="margin-bottom: 32px; display: flex; justify-content: space-between; align-items: center;">
        <div>
            <a href="<?php echo e(route('invoices.show', $invoice)); ?>" style="color: #64748b; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; margin-bottom: 12px; font-weight: 600;">
                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M15 19l-7-7 7-7"/>
                </svg>
                Back to Invoice
            </a>
            <h1 style="font-size: 32px; font-weight: 700; color: #1e293b; margin: 0;">Edit Invoice <?php echo e($invoice->invoice_number); ?></h1>
        </div>
    </div>
    ...
    <div style="background: white; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px rgba(0,0,0,0.05); overflow: hidden;">
        <form action="<?php echo e(route('invoices.update', $invoice)); ?>" method="POST" style="padding: 32px;">
            <?php echo csrf_field(); ?>
            ...
            <div style="margin-bottom: 32px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 20px;">
                    <div>
                        ...
                        <input type="text" name="customer_name" value="<?php echo e(old('customer_name', $invoice->customer_name)); ?>" required
                    ...
                    <div>
                        ...
                        <input type="email" name="customer_email" value="<?php echo e(old('customer_email', $invoice->customer_email)); ?>"
                               style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 20px;">
                    <div>
                        ...
                        <input type="text" name="customer_phone" value="<?php echo e(old('customer_phone', $invoice->customer_phone)); ?>"
                               style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;">
                    </div>
                    <div>
                        <label style="display: block; font-size: 14px; font-weight: 600; color: #64748b; margin-bottom: 8px;">Address</label>
                        <input type="text" name="customer_address" value="<?php echo e(old('customer_address', $invoice->customer_address)); ?>"
                               style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;">
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 32px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 24px; margin-bottom: 20px;">
                    <div>
                        ...
                        <select name="payment_method" required
                            ...
                            <option value="cash" <?php echo e(old('payment_method', $invoice->payment_method) == 'cash' ? 'selected' : ''); ?>>Cash</option>
                            <option value="card" <?php echo e(old('payment_method', $invoice->payment_method) == 'card' ? 'selected' : ''); ?>>Card</option>
                            <option value="bank_transfer" <?php echo e(old('payment_method', $invoice->payment_method) == 'bank_transfer' ? 'selected' : ''); ?>>Bank Transfer</option>
                            <option value="other" <?php echo e(old('payment_method', $invoice->payment_method) == 'other' ? 'selected' : ''); ?>>Other</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="payment_status" required
                                style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s; background: white;">
                            <option value="paid" <?php echo e(old('payment_status', $invoice->payment_status) == 'paid' ? 'selected' : ''); ?>>Paid</option>
                            <option value="unpaid" <?php echo e(old('payment_status', $invoice->payment_status) == 'unpaid' ? 'selected' : ''); ?>>Unpaid</option>
                            <option value="partial" <?php echo e(old('payment_status', $invoice->payment_status) == 'partial' ? 'selected' : ''); ?>>Partial</option>
                            <option value="cancelled" <?php echo e(old('payment_status', $invoice->payment_status) == 'cancelled' ? 'selected' : ''); ?>>Cancelled</option>
                        </select>
                    </div>
                    <div>
                        <label style="display: block; font-size: 14px; font-weight: 600; color: #64748b; margin-bottom: 8px;">Due Date</label>
                        <input type="date" name="due_date" value="<?php echo e(old('due_date', optional($invoice->due_date)->format('Y-m-d'))); ?>"
                               style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                              style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;"><?php echo e(old('notes', $invoice->notes)); ?></textarea>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/speed/resources/views/invoices/edit.blade.php ENDPATH**/ ?>