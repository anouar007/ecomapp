...
<head>
    ...
    <meta name="description" content="<?php echo $__env->yieldContent('meta_description', setting('app_description', 'Hijab Princesses — أناقة الأميرة: وجهتكم الفاخرة للعبايات والخمارات الراقية بالمغرب. جودة فاخرة وتوصيل سريع.')); ?>">
    ...
    <?php if(setting('app_logo')): ?>
        <link rel="icon" href="<?php echo e(Storage::url(setting('app_logo'))); ?>" type="image/x-icon">
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@200..1000&family=Playfair+Display:ital,wght@0,400..900;1,400..900&family=Inter:wght@300;400;500;600;700;800&family=Great+Vibes&family=Alex+Brush&display=swap" rel="stylesheet">
...
<body>
    ...
    <script>
        window.cardVariants = window.cardVariants || {};
        window.selectedCardOptions = window.selectedCardOptions || {};
        ...
        function addToCart(productId, isSlider = false) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    // Trigger FAB Show & Animation
                    updateFAB(data.cartCount, true);
                } else {
        ...
        function updateFAB(count, isNewAdd = false) {
            const fab = document.getElementById('checkout-fab');
            ...
            if (count > 0) {
                ...
                if (isNewAdd) {
                    fab.classList.remove('fab-pulse-animate');
                    void fab.offsetWidth; // Trigger reflow
                    fab.classList.add('fab-pulse-animate');
                }
            } else {
...
<!-- Go to Top Button -->
<button id="goTopBtn" class="go-top-btn d-none" title="<?php echo e(__('العودة إلى الأعلى')); ?>">
