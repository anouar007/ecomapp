...
<div class="page-header">
    <h1 class="page-title"><i class="fas fa-edit"></i> {{ __('Edit Customer') }}: {{ $customer->name }}</h1>
    <p class="page-subtitle">{{ __('Update customer information') }}</p>
</div>

<div class="card">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-file-alt"></i> {{ __('Customer Information') }}</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('customers.update', $customer) }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                <div class="form-group">
                    <label for="name" class="form-label">{{ __('Full Name') }} <span class="required">*</span></label>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control" required>
                        ...
                        <option value="inactive" {{ old('status', $customer->status) == 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                        <option value="blocked" {{ old('status', $customer->status) == 'blocked' ? 'selected' : '' }}>{{ __('Blocked') }}</option>
                    </select>
                </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> {{ __('Update Customer') }}
                </button>
            </div>
        </form>
    </div>
