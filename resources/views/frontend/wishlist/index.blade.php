...
@section('meta_title', __('My Wishlist') . ' — ' . setting('app_name', 'Bio Nature'))
...
<div class="bg-surface section-py min-vh-100">
    <div class="container px-xl-5">
        ...
        <div class="d-flex flex-column mb-5" data-aos="fade-up">
            <h1 class="brand-heading h2 mb-2">{{ __('Your Wishlist') }}</h1>
            ...
            <div class="col-6 col-md-4 col-lg-3" data-aos="fade-up">
                <div class="brand-card border-0 shadow-sm h-100 bg-white overflow-hidden hvr-float">
                    ...
                    <div class="card-body p-3 text-center">
                        ...
                        <a href="{{ route('shop.show', $product->id) }}" class="btn-brand-outline w-100 py-1 small text-decoration-none">{{ __('View Details') }}</a>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="text-center py-5" data-aos="fade-up">
            ...
            <h2 class="brand-heading mb-3">{{ __('Your wishlist is currently empty') }}</h2>
            <p class="text-muted mb-5 font-body text-center mx-auto" style="max-width: 400px;">{{ __('Do not let the most beautiful products and items pass you by. Add them here to shop later.') }}</p>
            <a href="{{ route('shop.index') }}" class="btn-brand-primary px-5 py-3 text-decoration-none hvr-grow">{{ __('Discover the collection now') }}</a>
        </div>
        @endif
    </div>
</div>
...
function removeFromWishlist(e, productId, btn) {
    Swal.fire({
        ...
        text: '{{ __("Do you want to remove this product from your wishlist?") }}',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if(data.status === 'removed') {
                    ...
                    Swal.fire({ toast:true, position:'top-end', icon:'success', title:'{{ __("Removed successfully") }}', showConfirmButton:false, timer:2000 });
                }
            })
            .catch(console.error);
        }
