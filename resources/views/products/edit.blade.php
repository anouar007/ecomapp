...
<div class="brand-header mb-4">
    <div>
        ...
        <p class="brand-subtitle">{{ __('Modify product details for') }} <span class="text-primary fw-bold">{{ $product->translated_name }}</span></p>
    </div>
        ...
        <div class="col-lg-8">
                    ...
                    <div class="tab-content" id="langTabsContent">
                        ...
                        <div class="tab-pane fade show active" id="ar-content" role="tabpanel">
                            <div class="form-group">
                                ...
                                <input type="text" name="name_ar" class="form-control form-control-lg" value="{{ old('name_ar', $product->name_ar) }}" placeholder="اسم المنتج بالكامل" dir="rtl" required autofocus>
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label">{{ __('Description (AR)') }}</label>
                                <textarea name="description_ar" class="form-control" rows="4" placeholder="وصف المنتج بالتفصيل..." dir="rtl">{{ old('description_ar', $product->description_ar) }}</textarea>
                            </div>
                        </div>
                        <!-- French -->
                        <div class="tab-pane fade" id="fr-content" role="tabpanel">
                            <div class="form-group">
                                ...
                                <input type="text" name="name_fr" class="form-control form-control-lg" value="{{ old('name_fr', $product->name_fr) }}" placeholder="Nom du produit">
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label">{{ __('Description (FR)') }}</label>
                                <textarea name="description_fr" class="form-control" rows="4" placeholder="Description détaillée en français...">{{ old('description_fr', $product->description_fr) }}</textarea>
                            </div>
                        </div>
                        <!-- English -->
                        <div class="tab-pane fade" id="en-content" role="tabpanel">
                            <div class="form-group">
                                ...
                                <input type="text" name="name_en" class="form-control form-control-lg" value="{{ old('name_en', $product->name_en) }}" placeholder="Product display name">
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label">{{ __('Description (EN)') }}</label>
                                <textarea name="description_en" class="form-control" rows="4" placeholder="Detailed English description...">{{ old('description_en', $product->description_en) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="cost_price" class="form-control border-start-0" value="{{ old('cost_price', $product->cost_price) }}" step="0.01" placeholder="0.00">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="price" class="form-control border-start-0 fw-bold" value="{{ old('price', $product->price) }}" step="0.01" placeholder="0.00" required>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="sale_price" class="form-control border-danger-subtle border-start-0 text-danger fw-bold" value="{{ old('sale_price', $product->sale_price) }}" step="0.01" placeholder="0.00">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <input type="number" name="min_stock" class="form-control" value="{{ old('min_stock', $product->min_stock) }}" required min="0">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card mb-4 border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                        ...
                        <div class="variant-card" data-id="{{ $variant->id }}">
                            ...
                            <div class="variant-header">
                                <div class="variant-img-wrapper" onclick="this.querySelector('input').click()">
                                    @if($variant->color_image)
                                        <img src="{{ asset('storage/' . $variant->color_image) }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 10px;">
                                    @else
                                        <i class="fas fa-camera text-muted"></i>
                                    @endif
                                    <input type="file" name="variants[{{ $index }}][color_image]" class="d-none" onchange="previewVariantImage(this)">
                                </div>
                                <div class="flex-grow-1">
                                    <h6 class="fw-bold mb-0" style="color: #475569;">{{ __('Variation') }} #{{ $index + 1 }}</h6>
                                </div>
                            ...
                            <div class="variant-grid-container">
                                <div class="variant-input-group">
                                    ...
                                    <input type="text" name="variants[{{ $index }}][size]" class="variant-input-xl" value="{{ $variant->size }}" placeholder="M, XL, 42...">
                                </div>

                                <div class="variant-input-group">
                                    <label class="variant-label-xl">{{ __('Grouping Key') }}</label>
                                    <input type="text" name="variants[{{ $index }}][style_key]" class="variant-input-xl" value="{{ $variant->style_key }}" placeholder="Set A">
                                </div>

                                <div class="variant-input-group">
                                    <label class="variant-label-xl">{{ __('Price') }} (DH)</label>
                                    <input type="number" name="variants[{{ $index }}][price]" class="variant-input-xl" value="{{ $variant->price }}" step="0.01">
                                </div>

                                <div class="variant-input-group">
                                    <label class="variant-label-xl text-danger">{{ __('Sale Price') }} (DH)</label>
                                    <input type="number" name="variants[{{ $index }}][sale_price]" class="variant-input-xl border-danger-subtle text-danger" value="{{ $variant->sale_price }}" step="0.01">
                                </div>

                                <div class="variant-input-group">
                                    <label class="variant-label-xl">{{ __('Stock') }}</label>
                                    <input type="number" name="variants[{{ $index }}][stock]" class="variant-input-xl" value="{{ $variant->stock }}">
                                </div>
                            </div>
                    ...
                    <div id="noVariantsMsg" class="text-center py-5 bg-light rounded-4 border-2 border-dashed {{ $product->variants->count() > 0 ? 'd-none' : '' }}">
...
let variantCounter = {{ $product->variants->count() + 100 }};
function addVariationRow() {
    ...
    div.innerHTML = `
        ...
        <div class="variant-header">
            ...
            <div class="flex-grow-1">
                <h4 class="fw-bold mb-1" style="color: #667eea;">{{ __('New Variation') }}</h4>
                <p class="text-muted mb-0">{{ __('Configuration for this specific item') }}</p>
            </div>
            <button type="button" class="btn btn-danger btn-lg rounded-pill" onclick="this.closest('.variant-card').remove(); checkVariantsEmpty();">
                <i class="fas fa-trash me-2"></i> {{ __('Remove') }}
            </button>
        </div>
        ...
        <div class="variant-grid-container">
            ...
            <div class="variant-input-group">
                <label class="variant-label-xl">{{ __('Regular Price') }} (DH)</label>
            ...
            <div class="col-md-4">
                <label class="variant-label-xl text-success">{{ __('Stock Quantity') }}</label>
