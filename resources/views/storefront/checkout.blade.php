...
<div><span>مصاريف التوصيل</span><b id="shipping-cost" @class(['checkout-free' => $shippingCost == 0])>{{ $shippingCost == 0 ? 'مجاني' : \App\Support\Storefront::money($shippingCost) }}</b></div>
<div class="checkout-total"><strong>المجموع الكلي</strong><b id="checkout-total">{{ \App\Support\Storefront::money($total + $shippingCost) }}</b></div>
</div>
...
<label><span>المدينة <em>*</em></span><select name="shipping_city" autocomplete="address-level2" required><option value="">اختر المدينة</option>@foreach(\App\Services\ShippingService::CITIES as $city)<option value="{{ $city }}" @selected(old('shipping_city') === $city)>{{ $city }}</option>@endforeach</select></label>
...
<script>
(() => {
    const quotes = {{ Illuminate\Support\Js::from($shippingQuotes) }};
    const city = document.querySelector('[name="shipping_city"]');
    const update = () => {
        const quote = quotes[city.value] || quotes[''];
        document.getElementById('shipping-cost').textContent = quote.shipping;
        document.getElementById('shipping-cost').classList.toggle('checkout-free', quote.free);
